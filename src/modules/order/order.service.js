...
export async function markKhqrPaymentSubmitted(orderId, { proofImage } = {}) {
  if (!proofImage) {
    throw createServiceError("Payment proof screenshot is required.", 400);
  }
  ...
  return runOrderTransaction(async (tx) => {
    ...
    if (existingOrder.payment?.status === "PENDING_VERIFICATION") {
      return existingOrder;
    }
    ...
    await createOrderTimelineEntries(
      ...
      [
        {
          type: "KHQR_PAYMENT_SUBMITTED",
          message: "Payment proof uploaded. Awaiting admin verification.",
          actorName: existingOrder.customerName || existingOrder.user?.email || "Customer"
        }
      ],
      tx
    );
